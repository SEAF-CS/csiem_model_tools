...
swanFile= 'W:\csiem\Model\TFV\BMT v3\A_HIND_2013_combined_base.nc';
...
for vv=1:length(swanVars)
    clf;
    tmpS=ncread(swanFile,swanVars{vv},[1 1 8753+tt],[Inf Inf 1]);
...
title({['SWAN A-HIND: ',datestr(swan.time(8753+tt),'yyyy-mm-dd HH:MM')],[swanVars{vv},': ',names2{vv}]});

subplot(1,2,2);
scatter(lon,lat,1,tmpW,'filled');
axis equal;
caxis([limLow(vv) limHigh(vv)]);
colorbar;
box on;
