...
for yy=2 %1:length(years2pro)
    ...
    for mm=1:length(m2p)
            wwmFile=[inDir,'output_Cgrid_',m2p{mm},'.mat'];
            disp(wwmFile);
            tmp=load(wwmFile);
            ...
            if mm==3
               inds=[1:216,219:744];
               tmp2=load(wwmFile);
               tmp.output.time=tmp2.output.time(inds);
               for vv=1:3
                    tmp.output.(swanVars{vv})=tmp2.output.(swanVars{vv})(:,:,inds);
               end
               clear tmp2; 
            else
            tmp=load(wwmFile);
