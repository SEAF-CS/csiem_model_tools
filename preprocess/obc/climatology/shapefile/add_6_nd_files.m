clear; close all;

shpfile='W:\csiem\Model\TFV\csiem_model_tfvaed_1.1\gis_repo\1_domain\nodestrings\ns_005_nutrient.shp';
shp=shaperead(shpfile);

hfig = figure('visible','on','position',[304         166        1271         812]);

set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'paperposition',[0.635 6.35 20.32 10]);

scatter(shp(1).X,shp(1).Y);
hold on;

for j=1:length(shp(1).X)-1
    text(shp(1).X(j),shp(1).Y(j),['ID',num2str(j)]);
    hold on;
end

for i=1:6
    infile=['Polygons_',num2str(i),'_MultiPolygon.shp'];
    tmpshp=shaperead(infile);
    pgon=polyshape(tmpshp.X,tmpshp.Y);
    plot(pgon,'FaceColor','red','FaceAlpha',0.1);
    hold on;
    XX=tmpshp.X; XXX=mean(XX(~isnan(XX)));
    YY=tmpshp.Y; YYY=mean(YY(~isnan(YY)));
    text(XXX,YYY,['polygon',num2str(i)])
end

axis equal;

saveas(gcf,'shape_check.png');

%% create new shp file for 6 polygons

shp2=shp;
shp2(10).ID=10;
shp2(11).ID=11;

sl=length(shp);

ids=[1 4 11 16 19 22 23];

for s=1:6
    
    newX=shp(1).X(ids(s):ids(s+1));
    newY=shp(1).Y(ids(s):ids(s+1));

    BoundingBox=[min(newX),min(newY);max(newX),max(newY)];
    newXX=[newX,NaN];
    newYY=[newY,NaN];
% 
%     if s==1
%         shp2(1).BoundingBox=BoundingBox;
%     shp2(1).X=newXX;
%     shp2(1).Y=newYY;
%    % shp2(1).ID=sl+s;
%     else
    shp2(sl+s)=shp2(1);

    shp2(sl+s).BoundingBox=BoundingBox;
    shp2(sl+s).X=newXX;
    shp2(sl+s).Y=newYY;
    shp2(sl+s).ID=sl+s;
%     end
end

shapewrite(shp2,'W:\csiem\Model\TFV\csiem_model_tfvaed_1.1\gis_repo\1_domain\nodestrings\ns_005_nutrient_6OBC_v2.shp')

