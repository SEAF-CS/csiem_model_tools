...
vars={'SAL','TEMP','WQ_DIAG_TOT_TN','WQ_DIAG_TOT_TP','WQ_PHS_FRP','WQ_NIT_AMM','WQ_NIT_NIT','WQ_OXY_OXY','WQ_DIAG_TOT_TSS','WQ_DIAG_PHY_TCHLA','WQ_OGM_DON'};
readdata=1;
...
if readdata
    ...
    for v=1:length(vars)
        v2i=vars{v};
        matfile=['..\datasets\DWER_data_',v2i,'.mat'];
        disp(matfile);
        data=load(matfile);
...
%         end
        ...
        for s=1:length(sites)
           ...
           if inpolygon(tmpdata2.X,tmpdata2.Y,shp(29).X,shp(29).Y)

           % tmpd=tmpdata2.Depth;
           % inds=find(tmpd>-30);

            tmpdepth=tmpdata2.Depth;
            tmpdate=tmpdata2.Date;
            tmpdata=tmpdata2.Data;
...
%             end
    ...
    save('processed_DWER_udates_allvars.mat','dataout','datasum','-mat','-v7.3');

else
    load processed_DWER_udates_allvars.mat;
end
...
titles={'Salinity','Temperature','TN','TP','FRP','AMM','NIT','DO','TSS','TCHLA'};
units={'PSU','degree C','\muM','\muM','\muM','\muM','\muM','\muM','mg/L','\mug/L'};
...
img_name ='check_DWER_udate_allvars.png';
...
img_name ='check_DWER_boxplot_allvars.png';
...
img_name ='check_DWER_boxchat_allvars.png';
