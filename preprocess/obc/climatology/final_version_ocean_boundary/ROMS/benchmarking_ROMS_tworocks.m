...
for v=1:length(vars)
    ...
    if v==1
        scatter(ROMS.merged.site1.time,ROMS.merged.site1.SAL,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',alp,'MarkerEdgeAlpha',alp,'DisplayName','ROMS');
        hold on;
    ...
    elseif v==2
        scatter(ROMS.merged.site1.time,ROMS.merged.site1.TEMP,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'DisplayName','ROMS');
        ylim([16 30]);
        hold on;
        hl=legend; 
...
for v=1:length(vars)
    ...
    if v==1
        scatter(ROMS.merged.site2.time,ROMS.merged.site2.SAL,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'DisplayName','ROMS');
        hold on;
    ...
    elseif v==2
        scatter(ROMS.merged.site2.time,ROMS.merged.site2.TEMP,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'DisplayName','ROMS');
        ylim([16 30]);
        hold on;
        hl=legend; 
        % set(hl,'Location','eastoutside');
...
for v=1:length(vars)
    subplot(3,2,v+4);
    scatter(CSIRO.csiem.A_SBE4534_10m.(vars{v}).Date,CSIRO.csiem.A_SBE4534_10m.(vars{v}).Data,ms,'filled','MarkerFaceColor',colors(1,:),'MarkerFaceAlpha',.6,'MarkerEdgeAlpha',.6,'DisplayName','CSIRO-nearshore');
    hold on;
    ...
    if v==1
        scatter(ROMS.merged.site3.time,ROMS.merged.site3.SAL,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'DisplayName','ROMS');
        ylim([34 38]);
        hold on;
        hl=legend; 
        set(hl,'Location','northwest');
    ...
    elseif v==2
        scatter(ROMS.merged.site3.time,ROMS.merged.site3.TEMP,ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.2,'MarkerEdgeAlpha',.2,'DisplayName','ROMS');
        ylim([16 30]);
        hold on;
        hl=legend; 
        % set(hl,'Location','eastoutside');
...
scatter(fData(ia), SData(ib)',ms,'filled','MarkerFaceColor',colors(1,:),'MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5);
[r,m,b] = regression(fData(ia)', SData(ib)','one');

hold on; box on; axis equal;
xlim=[35 37]; ylim=[35 37];
plot(xlim, ylim,'Color',clr);
hold on;
set(gca,'xlim',xlim,'ylim',ylim);
aa1=fData(ia);aa1(isnan(aa1))=mean(aa1(~isnan(aa1)));
aa2=SData(ib);aa2(isnan(aa2))=mean(aa2(~isnan(aa2)));
string={['r=',num2str(r,'%4.4f')],['bias=',num2str((mean(aa1)-mean(aa2))/mean(aa1)*100,'%4.2f'),'%']};
...
%sf = fit(fData(ia)', SData(ib)','poly1');
%plot(sf,fData(ia)', SData(ib)');
scatter(fData(ia), SData(ib)',ms,'filled','MarkerFaceColor',colors(2,:),'MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5);
[r,m,b] = regression(fData(ia)', SData(ib)','one');
...
hold on;
set(gca,'xlim',xlim,'ylim',ylim);
string={['r=',num2str(r,'%4.4f')],['bias=',num2str((mean(fData(ia))-mean(SData(ib)))/mean(fData(ia))*100,'%4.2f'),'%']};
xx=xlim(1)+(xlim(2)-xlim(1))*0.05;
yy=ylim(1)+(ylim(2)-ylim(1))*0.7;
hl=legend('data','1:1 ratio'); set(hl,'Location','northwest');
...
hfig4 = figure('visible','on','position',[304         166        1271         812]);
...
for j=1:length(tmpT2)
    t2i=tmpT2(j);
    tmpD2(j)=mean(tmpD(tmpT==t2i));
    tmpDC(j)=mean(newIMOSSAL(tmpT==t2i));
end
...
string={['r=',num2str(r,'%4.4f')],['bias=',num2str((mean(aa1)-mean(aa2))/mean(aa1)*100,'%4.2f'),'%']};
xx=xlim(1)+(xlim(2)-xlim(1))*0.05;
yy=ylim(1)+(ylim(2)-ylim(1))*0.68;
text(xx,yy,string,'Color',colors(2,:)); hold on;

hold on;

SDate=tmpT2; %floor(NASA.csiem.NASA_Polygon_3.TEMP.Date);
SData2=tmpDC; %NASA.csiem.NASA_Polygon_3.TEMP.Data;
[C, ia, ib]=intersect(fDate,SDate);
%scatter(fData(ia), SData(ib),ms,c2f,'filled','MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5);
hold on;
%sf = fit(fData(ia)', SData(ib)','poly1');
%plot(sf,fData(ia)', SData(ib)');
scatter(fData(ia), SData2(ib)',ms,'filled','MarkerFaceColor',colors(3,:),'MarkerFaceAlpha',.5,'MarkerEdgeAlpha',.5);
[r,m,b] = regression(fData(ia)', SData2(ib)','one');

hold on; box on; axis equal;

aa3=SData2(ib);aa3(isnan(aa3))=mean(aa3(~isnan(aa3)));
string={['r=',num2str(r,'%4.4f')],['bias=',num2str((mean(aa1)-mean(aa3))/mean(aa1)*100,'%4.2f'),'%']};
xx=xlim(1)+(xlim(2)-xlim(1))*0.05;
yy=ylim(1)+(ylim(2)-ylim(1))*0.52;
text(xx,yy,string,'Color',colors(3,:)); hold on;

plot(xlim, ylim,'Color',clr);
hold on;
hl=legend('data','data-corrected','1:1 ratio'); set(hl,'Location','northwest');
xlabel('IMOS'); ylabel('ROMS');
title('SAL - IMOS (PSU)');
...
for j=1:length(tmpT2)
    t2i=tmpT2(j);
    tmpD2(j)=mean(tmpD(tmpT==t2i));
    tmpDC(j)=mean(newDWERSAL(tmpT==t2i));
end
...
img_name ='ROMS_Benchmarking_regression_TwoRocks_corrected.png';
