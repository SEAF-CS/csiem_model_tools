...
% load IMOS
var='TSSorganic';
IMOS=load(['..\..\datasets\IMOS_data_',var,'.mat']);
...
for polys=1:6
    ...
    output.(['poly',num2str(polys)]).data=output.(['poly',num2str(3)]).data*scales(polys);
     output.(['poly',num2str(polys)]).data=movmean( output.(['poly',num2str(polys)]).data,6);
end
save([outDIR,'exported_POC.mat'],'raw','output','MOI','scales','-mat','-v7.3')
