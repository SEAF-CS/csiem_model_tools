...
timearray=IMOS.data.IMOS_BGC_NRSROT_BGC_Profile_49.WQ_NIT_NIT.Date;
AMM =IMOS.data.IMOS_BGC_NRSROT_BGC_Profile_49.WQ_NIT_NIT.Data;
...
save([outDIR,'exported_NIT.mat'],'raw','output','MOI','scales','-mat','-v7.3')
...
plot(output.(['poly',num2str(polys)]).time,...
  ...
  %  set(gca,'ylim',[215 245]);
    ...
    title(['NIT - poly',num2str(polys)]);
    hl=legend; %('IMOS2','IMOS5','IMOS11','IMOS61','CMEM');
    set(hl,'Location','eastoutside');


img_name =[outDIR,'timeseries_NIT_poly_',num2str(polys),'.png'];
