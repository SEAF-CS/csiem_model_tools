...
compiled_vars={'SS1','OXY','AMM','NIT','FRP','DOC','POC','CHLA'};
...
for polys=1:6
    disp(polys);
    fileID = fopen(['ocean_bgc_monthly_1980_2024_poly',num2str(polys),'_SAL.csv'],'w');
    fprintf(fileID,'%s\n',header);
...
for polys=1:6
    disp(polys);
    data = tfv_readBCfile(['ocean_bgc_monthly_1980_2024_poly',num2str(polys),'_SAL.csv']);

    outdir=['plots_BCs_SAL_poly',num2str(polys)];
