...
%       S(          13 ) AED pelagic(3D) variable: PHY_pico
%       S(          14 ) AED pelagic(3D) variable: PHY_mixed
%       S(          15 ) AED pelagic(3D) variable: PHY_diatom
%       S(          16 ) AED pelagic(3D) variable: PHY_dino
%       S(          17 ) AED pelagic(3D) variable: TRC_tr1
%       S(          18 ) AED pelagic(3D) variable: TRC_tr2
%       S(          19 ) AED pelagic(3D) variable: TRC_tr3
%       S(          20 ) AED pelagic(3D) variable: TRC_tr4
%       S(          21 ) AED pelagic(3D) variable: TRC_age

compiled_vars={'SAL','SS1','OXY','AMM','NIT','FRP','DOC','POC','CHLA'};
tmp=load('..\Ocean_second_round\SS\exported_SS.mat');
compiled.SS1=tmp.output;
tmp=load('..\Ocean_second_round\Dissolved_nutrients\exported_WQ_OXY_OXY.mat');
compiled.OXY=tmp.output;
tmp=load('..\Ocean_second_round\Dissolved_nutrients\exported_WQ_NIT_AMM.mat');
compiled.AMM=tmp.output;
tmp=load('..\Ocean_second_round\Dissolved_nutrients\exported_WQ_NIT_NIT.mat');
compiled.NIT=tmp.output;
tmp=load('..\Ocean_second_round\Dissolved_nutrients\exported_WQ_PHS_FRP.mat');
compiled.FRP=tmp.output;
tmp=load('..\Ocean_second_round\POC\exported_POC_3rd.mat');
compiled.POC=tmp.output;

tmp=load('..\Ocean_second_round\phytoplankton\exported_Diato.mat');
compiled.DIATOM=tmp.output;
tmp=load('..\Ocean_second_round\phytoplankton\exported_Dino.mat');
compiled.DINO=tmp.output;
tmp=load('..\Ocean_second_round\phytoplankton\exported_GREEN.mat');
compiled.GREEN=tmp.output;
tmp=load('..\Ocean_second_round\phytoplankton\exported_PROKAR.mat');
...
for polys=1:6
    disp(polys);
    fileID = fopen(['ocean_bgc_monthly_1980_2024_poly',num2str(polys),'_3round_capped.csv'],'w');
    fprintf(fileID,'%s\n',header);

    %cal DOC basd on POC using DOC=3.2408POC+3.4973 (mg dm-3, Maciejewska
    %2014)
    tmpdata0=compiled.POC.(['poly',num2str(polys)]).data;
%     tmpdata=tmpdata*12/1000;
%     DOC=tmpdata*3.2408+3.4973;
%     DOC=DOC/12*1000;
    mean0=mean(tmpdata0);

    for j=1:length(tmpdata0)
        tmpdata(j)=mean0+(tmpdata0(j)-mean0)/5;
    end
    compiled.POC.(['poly',num2str(polys)]).data=tmpdata;
    DOC=tmpdata*80/14;
...
for polys=1:6
    disp(polys);
    data = tfv_readBCfile(['ocean_bgc_monthly_1980_2024_poly',num2str(polys),'_3round_capped.csv']);
