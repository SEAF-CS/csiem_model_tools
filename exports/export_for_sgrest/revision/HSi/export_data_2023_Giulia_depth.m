...
for ll=1:8 %length(loadnames)
    loadname=loadnames{ll};
    disp(['loading ',loadname,' ...']);
    tic;
    
    rawdata=ncread(ncfile(1).name, loadname);
    output.(loadname).surface=rawdata(Surfcells,:);
    output.(loadname).bottom=rawdata(Bottcells,:);

    outfile=[outDir,'extracted_2023_',loadname,'.mat'];
    save(outfile,'output','-mat','-v7.3');clear output rawdata;
