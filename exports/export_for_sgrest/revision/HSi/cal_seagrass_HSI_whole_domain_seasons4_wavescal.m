...
t1=datenum(2023,10,1);
t2=datenum(2023,12,31);
...
faces(faces(:,4)== 0,4) = faces(faces(:,4)== 0,1);
     ...
     for i=1:length(pidcs)
         if i==1
         tmp=inpolygon(cellX, cellY, shp(pidcs(i)).X, shp(pidcs(i)).Y);
         else
             tmp=tmp+inpolygon(cellX, cellY, shp(i).X, shp(i).Y);
         end
     end
     tmp(tmp>1)=1;
     extcscale=1-tmp*0.5;
...
if readdata
	 ...
	 for i=1:size(D,1)
	     for j=1:size(D,2)
		     Lper0(i,j)=exp(-extc(i,j)*extcscale(i)*D(i,j));
		 end
     end
	 ...
	 if loadwave==1
		 ...
		 save('wave_2023_tmp4_wavescale.mat','waveorb','-mat','-v7.3');
	 else
		 load('wave_2023_tmp4_wavescale.mat');
	 end
	
     for i=1:size(waveorb,1)
         waveorbF(i)=prctile(waveorb(i,:)*0.5,98);
     end
     ...
     save('processed_2023_tmp4_wavescale.mat','sal','temp','vt','Lper','waveorbF','-mat','-v7.3');
else
     load('processed_2023_tmp4_wavescale.mat');
end
     ...
     hfig = figure('visible','on','position',[304         166         1800        1675]);

set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'centimeters');
set(gcf,'paperposition',[0.635 6.35 24 18]);
...
hc=colorbar;
set(hc,'Position',[0.8 0.12 0.02 0.3]);


img_name ='HSI_map_Oct-Dec2023_wavescale.png';
saveas(gcf,img_name);
