    ...
    for site=1:length(sitenames)
        ...
        if ll>=9
          %   tmp = tfv_getmodeldatalocation(ncfile(2).name,rawData.data,siteX(site),siteY(site),{loadname});
            ...
            tmp = rawData.data.(loadname)(Bottcells,:);
            
            output.(sitenames{site}).(loadname)=tmp(siteI(site),:);
        end
        clear tmp;
    
    end
    
    outfile=[outDir,'extracted_2023_',loadname,'_for_check.mat'];
...
pres=load('extracted_2023_WQ_DIAG_NCS_D_TAUB.mat');
new=load('extracted_2023_WQ_DIAG_NCS_D_TAUB_for_check.mat');
